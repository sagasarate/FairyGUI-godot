    ...
    public class SwipeGesture : EventDispatcher
    {
        ...
        public void Enable(bool value)
        {
            if (value)
            {
                if (host == GRoot.inst)
                {
                    GRoot.inst.onTouchBegin.Add(__touchBegin);
                    GRoot.inst.onTouchMove.Add(__touchMove);
                    GRoot.inst.onTouchEnd.Add(__touchEnd);
                }
                else
                {
            ...
            else
            {
                ...
                if (host == GRoot.inst)
                {
                    GRoot.inst.onTouchBegin.Remove(__touchBegin);
                    GRoot.inst.onTouchMove.Remove(__touchMove);
                    GRoot.inst.onTouchEnd.Remove(__touchEnd);
                }
                else
                {
        ...
        void __touchMove(EventContext context)
        {
            ...
            float elapsed = (curTime - _time) * 60 - 1;
            if (elapsed > 1) //速度衰减
                velocity = velocity * Mathf.Pow(0.833f, elapsed);
            velocity = velocity.Lerp(delta / deltaTime, deltaTime * 10);
            _time = curTime;
            position += delta;
            _lastPoint = pt;
        ...
        void __touchEnd(EventContext context)
        {
            ...
            //更新速度
            float elapsed = (Time.GetTicksMsec() / 1000.0f - _time) * 60 - 1;
            if (elapsed > 1)
                velocity = velocity * Mathf.Pow(0.833f, elapsed);
            if (snapping)
            {
