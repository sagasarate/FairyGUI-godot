...
namespace FairyGUI
{
    ...
    public class GPath
    {
        ...
        void CreateSplineSegment()
        {
            ...
            seg.length = 0;
            for (int i = 1; i < cnt; i++)
                seg.length += splinePoints[i - 1].DistanceTo(splinePoints[i]);
            _fullLength += seg.length;
            _segments.Add(seg);
            splinePoints.Clear();
        }
        ...
        Vector3 onCRSplineCurve(int ptStart, int ptCount, float t)
        {
            ...
            float adjustedT = (t == 1f) ? 1f : Mathf.PosMod(t * (ptCount - 4), 1f); // Then we adjust t to be that value on that new piece of segment... for t == 1f don't use repeat (that would return 0f);
