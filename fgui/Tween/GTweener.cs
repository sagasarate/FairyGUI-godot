...
namespace FairyGUI
{
    ...
    public class GTweener
    {
        ...
        internal void _Init()
        {
            ...
            _path = null;
            _customEase = null;
            _smoothStart = Engine.GetProcessFrames() == 1 ? 3 : 1;//刚启动时会有多帧的超时
        }
        ...
        internal void _Update(float deltaTime)
        {
            ...
            {
                _smoothStart--;
                dt = Mathf.Clamp(deltaTime, 0, Engine.MaxFps > 0 ? (1.0f / Engine.MaxFps) : 0.016f);
                if (!_ignoreEngineTimeScale)
                    dt *= (float)Engine.TimeScale;
            }
            else if (_ignoreEngineTimeScale)
                dt = deltaTime;
            else
                dt = deltaTime * (float)Engine.TimeScale;
            if (_timeScale != 1)
                dt *= _timeScale;
            if (dt == 0)
                return;
        ...
        void Update()
        {
            ...
            else if (_valueSize == 6)
            {
                if (_ended == 0)
                {
                    Vector3 r = RandomUtil.InsideUnitSphere();
                    r.X = r.X > 0 ? 1 : -1;
                    r.Y = r.Y > 0 ? 1 : -1;
                    r.Z = r.Z > 0 ? 1 : -1;
        ...
        void CallStartCallback()
        {
            if (GTween.catchCallbackExceptions)
            {
                ...
                {
                    GD.PushError("FairyGUI: error in start callback > " + e.Message);
                }
            }
            else
            {
        ...
        void CallUpdateCallback()
        {
            if (GTween.catchCallbackExceptions)
            {
                ...
                {
                    GD.PushError("FairyGUI: error in update callback > " + e.Message);
                }
            }
            else
            {
        ...
        void CallCompleteCallback()
        {
            if (GTween.catchCallbackExceptions)
            {
                ...
                {
                    GD.PushError("FairyGUI: error in complete callback > " + e.Message);
                }
            }
            else
            {
