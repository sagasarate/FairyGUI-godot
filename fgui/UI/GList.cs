...
namespace FairyGUI
{
    ...
    public class GList : GComponent
    {
        ...
        void SetSelectionOnEvent(GObject item, NInputEvent evt)
        {
        ...
        void SetVirtual(bool loop)
        {
            ...
            {
                if (this.scrollPane == null)
                    GD.PushError("FairyGUI: Virtual list must be scrollable!");

                if (loop)
                {
                    if (_layout == ListLayoutType.FlowHorizontal || _layout == ListLayoutType.FlowVertical)
                        GD.PushError("FairyGUI: Loop list is not supported for FlowHorizontal or FlowVertical layout!");
                ...
                if (_itemSize.X == 0 || _itemSize.Y == 0)
                {
                    ...
                    if (obj == null)
                    {
                        GD.PushError("FairyGUI: Virtual List must have a default list item resource.");
        ...
        void HandleScroll(bool forceUpdate)
        {
            ...
            if (_layout == ListLayoutType.SingleColumn || _layout == ListLayoutType.FlowHorizontal)
            {
                ...
                while (HandleScroll1(forceUpdate))
                {
                    ...
                    if (enterCounter > 20)
                    {
                        GD.PushWarning("FairyGUI: list will never be filled as the item renderer function always returns a different size.");
                        break;
                    }
                }
        ...
        bool HandleScroll1(bool forceUpdate)
        {
            ...
            if (curIndex > 0 && this.numChildren > 0 && this.container.Position.Y <= 0 && GetChildAt(0).y > -this.container.Position.Y)//最后一页没填满！
                return true;
            else
                return false;
        }
        ...
        bool HandleScroll2(bool forceUpdate)
        {
            ...
            if (curIndex > 0 && this.numChildren > 0 && this.container.Position.X <= 0 && GetChildAt(0).x > -this.container.Position.X)//最后一页没填满！
                return true;
            else
                return false;
        }
