...
namespace FairyGUI
{
    ...
    public class GSlider : GComponent
    {
        ...
        private void UpdateWithPercent(float percent, bool manual)
        {
            ...
            if (manual)
            {
                ...
                if (_wholeNumbers)
                {
                    ...
                    percent = Mathf.Clamp((float)((newValue - _min) / (_max - _min)), 0f, 1f);
                }
        ...
        private void __gripTouchBegin(EventContext context)
        {
            ...
            _clickPos = this.ViewportToLocal(new Vector2(evt.x, evt.y));
            _clickPercent = Mathf.Clamp((float)((_value - _min) / (_max - _min)), 0f, 1f);
        }
        ...
        private void __gripTouchMove(EventContext context)
        {
            ...
            NInputEvent evt = context.inputEvent;
            Vector2 pt = this.ViewportToLocal(new Vector2(evt.x, evt.y));
            if (float.IsNaN(pt.X))
                return;
        ...
        private void __barTouchBegin(EventContext context)
        {
            ...
            float percent = Mathf.Clamp((float)((_value - _min) / (_max - _min)), 0f, 1f);
            float delta = 0;
            if (_barObjectH != null)
