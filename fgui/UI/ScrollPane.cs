...
namespace FairyGUI
{
    ...
    public class ScrollPane : EventDispatcher
    {
        ...
        Action<double> _tweenUpdateDelegate;
        float _lastTimerTriggerTime;
        ...
        public ScrollPane(GComponent owner, NContainer clipContainer)
        {
            ...
            _maskContainer = clipContainer;

            _container = _owner.container;

            _owner.onMouseWheel.Add(__mouseWheel);
            _owner.onTouchBegin.Add(__touchBegin);
            _owner.onTouchMove.Add(__touchMove);
            _owner.onTouchEnd.Add(__touchEnd);
        }
        ...
        public void Setup(ByteBuffer buffer)
        {
            ...
            if (scrollBarDisplay != ScrollBarDisplayType.Hidden)
            {
                if (_scrollType == ScrollType.Both || _scrollType == ScrollType.Vertical)
                {
                    ...
                    if (!string.IsNullOrEmpty(res))
                    {
                        ...
                        if (_vtScrollBar == null)
                            GD.PushWarning("FairyGUI: cannot create scrollbar from " + res);
                        else
                        {
                            _vtScrollBar.SetScrollPane(this, true);
                            _owner.AddInternalChild(_vtScrollBar, false);
                        }
                    }
                }
                if (_scrollType == ScrollType.Both || _scrollType == ScrollType.Horizontal)
                {
                    ...
                    if (!string.IsNullOrEmpty(res))
                    {
                        ...
                        else
                        {
                            ...
                            _owner.AddInternalChild(_hzScrollBar, false);
                        }
                    }
                }
                ...
                if (_scrollBarDisplayAuto)
                {
                    ...
                    _owner.onRollOver.Add(__rollOver);
                    _owner.onRollOut.Add(__rollOut);
                }
            }
            else
            ...
            if (headerRes != null)
            {
                ...
                if (_header == null)
                    GD.PushWarning("FairyGUI: cannot create scrollPane header from " + headerRes);
            }
            ...
            if (footerRes != null)
            {
                ...
                if (_footer == null)
                    GD.PushWarning("FairyGUI: cannot create scrollPane footer from " + footerRes);
            }
        ...
        public void SetPercX(float value, bool ani)
        {
            ...
            SetPosX(_overlapSize.X * Mathf.Clamp(value, 0, 1), ani);
        }
        ...
        {
            _owner.EnsureBoundsCorrect();
            SetPosY(_overlapSize.Y * Mathf.Clamp(value, 0, 1), ani);
        }
        ...
        public float scrollingPosX
        {
            get
            {
                return Mathf.Clamp(-_container.Position.X, 0, _overlapSize.X);
            }
        }
        ...
        public float scrollingPosY
        {
            get
            {
                return Mathf.Clamp(-_container.Position.Y, 0, _overlapSize.Y);
            }
        }
        ...
        public bool IsChildInView(GObject obj)
        {
            if (_overlapSize.Y > 0)
            {
                float dist = obj.y + _container.Position.Y;
                if (dist <= -obj.height || dist >= _viewSize.Y)
                    return false;
            }
            if (_overlapSize.X > 0)
            {
                float dist = obj.x + _container.Position.X;
        ...
        {
            Stage.inst.RemoveTouchMonitor(_owner);
        ...
        public void LockHeader(int size)
        {
            ...
            if (!isDispatching("onPullDownRelease") && _container.Position[_refreshBarAxis] >= 0)
            {
                _tweenStart = _container.Position;
        ...
        public void LockFooter(int size)
        {
            ...
            if (!isDispatching("onPullUpRelease") && _container.Position[_refreshBarAxis] <= -_overlapSize[_refreshBarAxis])
            {
                _tweenStart = _container.Position;
                _tweenChange = Vector2.Zero;
        ...
        {
            if (Mathf.IsEqualApprox(_contentSize.X, aWidth) && Mathf.IsEqualApprox(_contentSize.Y, aHeight))
                return;
        ...
        void Refresh2()
        {
            if (_aniFlag == 1 && !_dragged)
            {
                ...
                if (pos.X != _container.Position.X || pos.Y != _container.Position.Y)
                {
        ...
        private void __touchBegin(EventContext context)
        {
            ...
            Vector2 pt = _owner.ViewportToLocal(evt.position);
            ...
            _velocityScale = 1;
            _lastMoveTime = Time.GetTicksMsec() / 1000.0f;
        }
        ...
        private void __touchMove(EventContext context)
        {
            ...
            Vector2 CurPos = _container.Position;
            if (sv)
            {
                if (newPos.Y > 0)
                {
                    if (!_bouncebackEffect)
                        ...
                        CurPos.Y = (int)Mathf.Min(newPos.Y * 0.5f, _header.maxHeight);
                    else
                        CurPos.Y = (int)Mathf.Min(newPos.Y * 0.5f, _viewSize.Y * PULL_RATIO);
                }
                else if (newPos.Y < -_overlapSize.Y)
                {
                    if (!_bouncebackEffect)
                        CurPos.Y = -_overlapSize.Y;
                    else if (_footer != null && _footer.maxHeight > 0)
                        CurPos.Y = (int)Mathf.Max((newPos.Y + _overlapSize.Y) * 0.5f, -_footer.maxHeight) - _overlapSize.Y;
                    else
                        CurPos.Y = (int)Mathf.Max((newPos.Y + _overlapSize.Y) * 0.5f, -_viewSize.Y * PULL_RATIO) - _overlapSize.Y;
                }
                else
                    CurPos.Y = newPos.Y;
            }
            ...
            if (sh)
            {
                if (newPos.X > 0)
                {
                    if (!_bouncebackEffect)
                        ...
                        CurPos.X = (int)Mathf.Min(newPos.X * 0.5f, _header.maxWidth);
                    else
                        CurPos.X = (int)Mathf.Min(newPos.X * 0.5f, _viewSize.X * PULL_RATIO);
                }
                else if (newPos.X < 0 - _overlapSize.X)
                {
                    if (!_bouncebackEffect)
                        CurPos.X = -_overlapSize.X;
                    else if (_footer != null && _footer.maxWidth > 0)
                        CurPos.X = (int)Mathf.Max((newPos.X + _overlapSize.X) * 0.5f, -_footer.maxWidth) - _overlapSize.X;
                    else
                        CurPos.X = (int)Mathf.Max((newPos.X + _overlapSize.X) * 0.5f, -_viewSize.X * PULL_RATIO) - _overlapSize.X;
                }
                else
                    CurPos.X = newPos.X;
            }
            ...
            float deltaTime = CurTime - _lastMoveTime;
            float elapsed = (CurTime - _lastMoveTime) * 60 - 1;
            if (elapsed > 1) //速度衰减
            ...
            //同步更新pos值
            if (_overlapSize.X > 0)
                _xPos = Mathf.Clamp(-_container.Position.X, 0, _overlapSize.X);
            if (_overlapSize.Y > 0)
                _yPos = Mathf.Clamp(-_container.Position.Y, 0, _overlapSize.Y);

            //循环滚动特别检查
            if (_loop != 0)
            {
                newPos = _container.Position;
                if (LoopCheckingCurrent())
                    _containerPos += _container.Position - newPos;
            }
        ...
        private void __touchEnd(EventContext context)
        {
            ...
            else if (_container.Position.X < -_overlapSize.X)
            {
            ...
            else if (_container.Position.Y < -_overlapSize.Y)
            {
            ...
            if (flag)
            {
                ...
                _tweenDuration.X = TWEEN_TIME_DEFAULT;
                _tweenDuration.Y = TWEEN_TIME_DEFAULT;
            }
            else
            {
                //更新速度
                if (!_inertiaDisabled)
                {
                    float elapsed = (Time.GetTicksMsec() / 1000.0f - _lastMoveTime) * 60 - 1;
                    if (elapsed > 1)
                        _velocity = _velocity * Mathf.Pow(0.833f, elapsed);
        ...
        {
            if (_vtScrollBar != null)
                _vtScrollBar.setScrollPerc(_overlapSize.Y == 0 ? 0 : Mathf.Clamp(-_container.Position.Y, 0, _overlapSize.Y) / _overlapSize.Y);

            if (_hzScrollBar != null)
                _hzScrollBar.setScrollPerc(_overlapSize.X == 0 ? 0 : Mathf.Clamp(-_container.Position.X, 0, _overlapSize.X) / _overlapSize.X);
        ...
        float AlignByPage(float pos, int axis, bool inertialScrolling)
        {
            ...
            else
            {
                ...
                float change = inertialScrolling ? (pos - _containerPos[axis]) : (pos - _container.Position[axis]);
        ...
        float UpdateTargetAndDuration(float pos, int axis)
        {
            ...
            else
            {
                ...
                if (Stage.touchScreen)
                    v2 *= 1136f / Mathf.Max(DisplayServer.WindowGetSize().X, DisplayServer.WindowGetSize().Y);
        ...
        void CheckRefreshBar()
        {
            ...
            float pos = _container.Position[_refreshBarAxis];
            if (_header != null)
            {
                if (pos > 0)
                {
                    if (_header.displayObject.node.GetParent() == null)
                    {
                        _owner.AddInternalChild(_header, false);
                    }
                ...
                {
                    if (_header.displayObject.node.GetParent() != null)
                        _owner.RemoveInternalChild(_header, false);
                }
            }
            ...
            if (_footer != null)
            {
                ...
                {
                    if (_footer.displayObject.node.GetParent() == null)
                    {
                        _owner.AddInternalChild(_footer, false);
                    }
                ...
                {
                    if (_footer.displayObject.node.GetParent() != null)
                        _owner.RemoveInternalChild(_footer, false);
                }
            }
        }

        void TweenUpdate(double delta)
        {
            if (_owner.displayObject == null || _owner.displayObject.node.IsQueuedForDeletion())
            {
                _tweening = 0;
        ...
        float RunTween(int axis, double delta)
        {
            float newValue;
            if (_tweenChange[axis] != 0)
            {
                _tweenTime[axis] += (float)delta;
                ...
                newValue = _container.Position[axis];
