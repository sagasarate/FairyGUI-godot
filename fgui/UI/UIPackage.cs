...
namespace FairyGUI
{
    ...
    public class UIPackage
    {
        ...
        /// <param name="type">Resource type. e.g. 'Texture2D' 'AudioClip'</param>
        /// <param name="destroyMethod">How to destroy this resource.</param>
        /// <returns></returns>
        public delegate object LoadResource(string name, System.Type type, out DestroyMethod destroyMethod);
        ...
        /// <returns></returns>
        public delegate void LoadResourceAsync(string name, System.Type type, PackageItem item);
        ...
        public const string RES_PREFIX = "res://";

        static LoadResource _loadFromResourcesPath = (string path, Type type, out DestroyMethod destroyMethod) =>
        {
            ...
            if (type == typeof(byte[]))
            {
                ...
                {
                    GD.PushWarning($"UI Package File not found: {path}");
                    return null;
                }
                ...
                if (file.GetLength() == 0)
                {
                    GD.PushWarning($"Invalid UI Package File: {path}");
                    return null;
                }
                byte[] res = file.GetBuffer((long)file.GetLength());
                file.Close();
                return res;
            }
            else
            {
                ...
                if (res != null && type != null && !type.IsInstanceOfType(res))
                {
                    GD.PrintErr($"Resource at {path} is not of type {type}");
                    return null;
                }
                return res;
            }
        ...
        {
            return AddPackage(assetPath, _loadFromResourcesPath);
        }
        ...
        public static UIPackage AddPackage(string assetPath, LoadResource loadFunc)
        {
            ...
            DestroyMethod dm;
            byte[] asset = loadFunc(assetPath, typeof(byte[]), out dm) as byte[];
            if (asset == null)
                GD.PushError("FairyGUI: Cannot load ui package in '" + assetPath + "'");

            ByteBuffer buffer = new ByteBuffer(asset);
        ...
        public static void CreateObjectAsync(string pkgName, string resName, CreateObjectCallback callback)
        {
            ...
            if (pkg != null)
                ...
                GD.PushError("FairyGUI: package not found - " + pkgName);
        }
        ...
        public static void CreateObjectFromURL(string url, CreateObjectCallback callback)
        {
            PackageItem pi = GetItemByURL(url);
            if (pi != null)
                AsyncCreationHelper.Instance.CreateObject(pi, callback);
            else
                GD.PushError("FairyGUI: resource not found - " + url);
        }
        ...
        bool LoadPackage(ByteBuffer buffer, string assetNamePrefix)
        {
            ...
            {
                GD.PushWarning("FairyGUI: old package format found in '" + assetNamePrefix + "'");
                return false;
            }
            ...
            string assetPath = assetNamePrefix.GetBaseDir();
            if (assetPath.Length > 0)
                assetPath += "/";
            assetNamePrefix = assetNamePrefix.GetBaseName();
            int pos = assetNamePrefix.IndexOf("_fui");
            if (pos != -1)
                assetNamePrefix = assetNamePrefix.Substring(0, pos);
            PackageItem pi;
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                switch (pi.type)
                {
                    ...
                    case PackageItemType.Atlas:
                        {
                            pi.file = assetNamePrefix + pi.file + ".png";
                            break;
                        }
                    case PackageItemType.Sound:
                        {
                            pi.file = assetNamePrefix + pi.file + ".wav";
                            break;
                        }
                    case PackageItemType.Misc:
                        {
                            pi.file = assetNamePrefix + pi.file + ".ogg";
                            break;
                        }
        ...
        public GObject CreateObject(string resName)
        {
            ...
            {
                GD.PushError("FairyGUI: resource not found - " + resName + " in " + this.name);
                return null;
            }
        ...
        public bool SetItemAsset(PackageItem item, object asset, DestroyMethod destroyMethod)
        {
            switch (item.type)
            {
                case PackageItemType.Atlas:
                    ...
                    item.texture.Reload((Texture2D)asset, null);
                ...
                case PackageItemType.Sound:
                    ...
                    item.audioClip.Reload((AudioStream)asset);
        ...
        void LoadAtlas(PackageItem item)
        {
            ...
            {
                _loadAsyncFunc(fileName, typeof(Texture2D), item);
                if (item.texture == null)
                    item.texture = new NTexture(null, new Rect(0, 0, item.width, item.height));
            ...
            else
            {
                ...
                tex = (Texture2D)_loadFunc(fileName, typeof(Texture2D), out dm);

                if (tex == null)
                    GD.PushWarning("FairyGUI: texture '" + item.file + "' not found in " + this.name);

                else if (!(tex is Texture2D))
                {
                    GD.PushWarning("FairyGUI: settings for '" + item.file + "' is wrong! Correct values are: (Texture2D Type=Default, Texture2D Shape=2D)");
                    tex = null;
                }
                ...
                if (item.texture == null)
                {
                    item.texture = new NTexture(tex, alphaTex, (float)tex.GetWidth() / item.width, (float)tex.GetHeight() / item.height);
        ...
        void LoadSound(PackageItem item)
        {
            ...
            if (_loadAsyncFunc != null)
            {
                _loadAsyncFunc(fileName, typeof(AudioStream), item);
                if (item.audioClip == null)
                    item.audioClip = new NAudioClip(null);
            ...
            else
            {
                ...
                audioClip = _loadFunc(fileName, typeof(AudioStream), out dm) as AudioStream;
        ...
        {
            if (!FileAccess.FileExists(item.file))
            {
                GD.PushWarning($"UI Package File not found: {item.file}");
                return null;
            }

            var file = FileAccess.Open(item.file, FileAccess.ModeFlags.Read);
            if (file.GetLength() == 0)
            {
                GD.PushWarning($"Invalid UI Package File: {item.file}");
                return null;
            }
            byte[] res = file.GetBuffer((long)file.GetLength());
            file.Close();
        ...
        void LoadSpine(PackageItem item)
        {
            ...
            GD.PushWarning("Spine not support");
        }

        void LoadDragonBones(PackageItem item)
        {
            GD.PushWarning("DragonBones not upport");
        }
